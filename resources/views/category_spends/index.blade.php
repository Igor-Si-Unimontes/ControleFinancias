...
@section('title', 'Categoria de Gastos')

@section('content')
    <div class="container rounded bg-fosco shadow-sm mt-4 p-5">
        <div class="d-flex justify-content-between align-items-center mb-2">
            <h1 class="mb-0 titulo">Lista de Categorias de Gastos</h1>
            <a href="{{ route('category_spends.create') }}" class="btn btn-success">Adicionar Nova Categoria</a>
        </div>
        ...
        <table id="spendsTable" class="table table-hover bg-fosco">
            ...
            <tbody>
                @foreach ($spends as $categorySpend)
                    <tr>
                        <td style="width: 75%;">{{ $categorySpend->name }}</td>
                        <td style="width: 25%;" class="text-center">
                            <a href="{{ route('category_spends.edit', $categorySpend->id) }}" class="btn btn-sm"
                                style="color: #43a047;" title="Editar">
                            ...
                            <a href="#" class="btn btn-sm" data-bs-toggle="modal"
                                data-bs-target="#deleteCategorySpendModal-{{ $categorySpend->id }}" style="color: #DC2626;"
                                title="Apagar">
                                <svg xmlns="http://www.w3.org/2000/svg" class="lucide" width="24" height="24"
                    ...
                    <div class="modal fade" id="deleteCategorySpendModal-{{ $categorySpend->id }}" tabindex="-1"
                        aria-labelledby="deleteCategorySpendModalLabel-{{ $categorySpend->id }}" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="deleteCategorySpendModalLabel-{{ $categorySpend->id }}">
                                        Confirmar
                                        Exclusão</h5>
                                ...
                                <div class="modal-body">
                                    Tem certeza que deseja excluir a categoria <strong>{{ $categorySpend->name }}</strong>?
                                </div>
                                <div class="modal-footer">
                                    <form action="{{ route('category_spends.destroy', $categorySpend->id) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
